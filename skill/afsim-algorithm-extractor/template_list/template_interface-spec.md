...
1. **中文注释全覆盖**：接口规格文件应当是可供只会中文的新手开发人员阅读和理解的产物，本文所有章节都必须添加中文注释，且避免复杂高深的表述。
2. **语言统一**：文档中不同章节在表示相同内容时使用的标识符和名称应当统一，语言风格应当保持严谨准确。
3. 每个 `struct` / `class` 开头的注释说明其总体接口职责。
4. 每个方法和成员变量附带中文注释说明其物理意义和单位。
5. 典型调用示例（示例代码块）中的每行/每段代码附带中文注释。
--- 
...
> **日期:** 2026-06-10
> **状态:** draft
> **对应算法卡:** p6dof-heun-integrator-card.md, pointmass-integration-sas-card.md
