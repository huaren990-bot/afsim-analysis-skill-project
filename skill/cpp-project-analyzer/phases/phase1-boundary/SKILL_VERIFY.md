...
1. 检查 `project-boundary.json` 是否记录 `source` + `header` 文件数量，且与 `file-classification.jsonl` 中统计一致。
2. 检查是否记录 `compile_commands.json` 状态：
   - 若存在，必须有路径和编译单元数量。
   - 若不存在，`notes` 中必须说明后续 AST/编译参数证据不可用。
3. 如果源码/头文件数量超过 1000，检查 `notes` 是否提示后续按模块/子目录分片。

## 输出

生成验证报告 `docs/verification/phase1-verify-report.md`，包含：
