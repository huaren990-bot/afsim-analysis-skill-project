...
> **单位策略**：内部计算全部使用 SI 单位制，接口输入输出使用 SI 单位。在关键公式注释中标注原始 AFSIM Imperial 值以保持可追溯性。**⚠️ FU-006（Autopilot PID）例外**：若 SI 单位转换后 PID 增益需要重调，该 FU 内部保留 Imperial 单位，仅在对外接口（FU-009 集成调度器）处进行单位转换。
...
sequenceDiagram
    ...
    Scheduler->>+FU2: ② advanceAlongPath(leg_index, leg_progress, V, dt, lonwind, latwind, altwind, path, speed_profile)
    Note over FU2: 三维风矢量修正+ECS坐标系位移<br/>+高度线性插值+速度查表
...
| ② | `advanceAlongPath()` | FU-002 | 航段索引+段内进度+速度+步长+三维风矢量+航线+速度规划 | → 步骤③⑥⑦ |
...
- **✅ 已确认——地球曲率**：增加 `use_curvature` 标志。当航段跨度 > 100 km（可通过 `Earth_Params` 可配置阈值）或全局开启时，距离计算使用 Haversine 公式替代平面近似：
  $$a = \sin^2(\Delta lat/2) + \cos(lat_1) \cdot \cos(lat_2) \cdot \sin^2(\Delta lon/2)$$
  $$d = 2 \cdot R_{earth} \cdot \text{atan2}(\sqrt{a}, \sqrt{1-a})$$
...
```mermaid
flowchart TD
    A([开始 advanceAlongPath]) --> B["输入: leg_index, leg_progress, V, dt, lonwind, latwind, altwind, path, speed_profile"]
    B --> C["① 计算三维风矢量沿航段投影<br/>V_wind_proj = lonwind·d_lon + latwind·d_lat + altwind·d_alt<br/>V_eff = V + V_wind_proj"]
...
计算三维风矢量沿航段方向投影修正后的有效速度和进度增量的公式如下：
$$V_{wind\_proj} = u_{lon} \cdot \hat{d}_{lon} + u_{lat} \cdot \hat{d}_{lat} + u_{alt} \cdot \hat{d}_{alt}$$
$$V_{eff} = V + V_{wind\_proj}$$
$$\Delta p = \frac{V_{eff} \cdot \Delta t}{L_{leg}}$$
$$\text{new\_progress} = \text{leg\_progress} + \Delta p$$
其中 $\hat{\mathbf{d}}_{leg} = (\hat{d}_{lon}, \hat{d}_{lat}, \hat{d}_{alt})$ 为航路段方向单位向量（由 Point 经纬高坐标差归一化得到），$L_{leg} = |\overrightarrow{P_iP_{i+1}}|$ 为段长度（m），$u_{lon}$ 为经向风（南北，北为正）、$u_{lat}$ 为纬向风（东西，东为正）、$u_{alt}$ 为垂直风（向上为正），$V_{eff}$ 为风速沿航路段方向投影修正后的有效速度。
...
- **函数功能**：为实现航线推进算法中的"参考点推进+三维指令输出"部分，`advanceAlongPath` 将航路段索引、段内进度、速度、步长和风速等输入转化为下一时刻参考点位置和三轴制导指令（heading/altitude/speed），支持跨段 推进和终点停留。
...
- **复杂度**：O(1)——常数次向量运算和插值
- **签名**：`AdvanceOutput advanceAlongPath(int leg_index, double leg_progress, double V, double dt, double lonwind, double latwind, double altwind, const std::vector<Point>& path, const std::vector<double>& speed_profile);`
...
| 5   | `lonwind`       | `double`                     | 任意实数 m/s           | 经向风（南北风向，北为正），单位 m/s     |
| 6   | `latwind`       | `double`                     | 任意实数 m/s           | 纬向风（东西方向，东为正），单位 m/s     |
| 7   | `altwind`       | `double`                     | 任意实数 m/s           | 垂直风（沿地球重力方向，向上为正），单位 m/s |
| 8   | `path`          | `const std::vector<Point>&`  | size ≥ 2           | 期望航线航路点数组                |
| 9   | `speed_profile` | `const std::vector<double>&` | size = path.size() | 每个航路点的期望巡航速度（m/s）        |
...
| 风矢量导致大幅跳跃 | `|delta_progress| > 2.0` | clamp 到 2.0（最多跨越 2 段），打印警告 | 限幅后值 |
| 空速度规划（size != path.size()） | 入口检查 | 使用默认速度 V_max/2，打印警告 | 默认速度 |

### 风险与未决问题
- **技术风险**：地理坐标系下位移计算需正确处理经纬度→距离转换，小范围假设（平面近似）可能在大跨度场景引入误差
- **✅ 已确认——大跨度地球曲率**：航段长度 $L_{leg}$ 计算增加曲率模式。当航段跨度 > 100 km 时，使用 Haversine 公式计算大圆距离；航向角 $heading_{cmd}$ 使用球面正向方位角公式替代平面 atan2：
  $$\theta = \text{atan2}(\sin(\Delta lon) \cdot \cos(lat_2), \cos(lat_1) \cdot \sin(lat_2) - \sin(lat_1) \cdot \cos(lat_2) \cdot \cos(\Delta lon))$$
  参考点推进使用球面线性插值（slerp）沿大圆路径移动，替代 Cartesian lerp。小跨度场景默认使用平面近似以保持计算效率。
...
简化版推进系统——采用三个简化方案的组合：(1) **简1**线性推力-油门关系 T=δ×T(altitude)，推力通过 1D 曲线 `InterpCurve1D` 按高度插值查表（跳过 AFSIM 完整三层查表 Idle/Mil/AB + spool dynamics）；(2) **简2**恒定燃油消耗率 m_fuel=ṁ_const×Δt（跳过 TSFC 查表和多工况切换）；(3) **简3**单油箱直接消耗模型（跳过 AFSIM 多油箱传输协调和 CG 位置插值）。该 FU 是 KINEMATICS-02 管线的第一级，为积分器（FU-008）提供推力输入和更新后的燃油质量。目标系统为空系统。算法卡片：[jet-engine-card](../../algorithms/flight-dynamics-jet-engine-card.md) + [propulsion-fuel-card](../../algorithms/flight-dynamics-propulsion-fuel-card.md) 已逐卡阅读。
...
flowchart TD
    ...
    C -->|否| D["② 1D曲线查表 T(altitude)<br/>InterpCurve1D 按高度插值"]
    D --> E["③ 线性推力计算<br/>F_thrust = throttle × T(altitude)"]
    E --> F{"④ throttle == 0?"}
...
1. （对应到流程图中的流程②③）：1D 推力曲线查表[引用](../../algorithms/flight-dynamics-jet-engine-card.md)
计算推力的公式如下（简1）：
$$T(altitude) = \text{InterpCurve1D}(altitude, \text{thrust\_curve})$$
$$F_{thrust} = \delta_{throttle} \cdot T(altitude)$$
其中 $\delta_{throttle} \in [0, 1]$ 为油门位置（0=慢车/1=全推力），$T(altitude)$ 为通过 1D 曲线 `InterpCurve1D` 按当前高度线性插值得到的推力值（N）。推力曲线数据来自 AFSIM 默认 Mil 推力表经 Imperial→SI 转换。
...
| #   | 名称                  | 类型         | 来源                                       | 有效范围/约束       | 说明                          |
| --- | ------------------- | ---------- | ---------------------------------------- | ------------- | --------------------------- |
| 1   | `T(altitude)`       | `InterpCurve1D` | 配置文件（AFSIM 默认 Mil 推力表，Imperial→SI 转换） | [0, 500000] N | 1D 推力曲线 f(altitude)，按当前高度线性插值获取推力值（N） |
| 2   | `m_dot_const`       | `double`   | `para.getPara("m_dot_const", 0.0)`       | (0, 100] kg/s | 恒定燃油质量流量                    |
| 3   | `Max_Fuel_Capacity` | `double`   | `para.getPara("Max_Fuel_Capacity", 0.0)` | (0, 50000] kg | 油箱最大容量                      |
...
| 控制耦合 | 无——纯数值计算，无状态机依赖 |
| 外部依赖 | 低——T(altitude) 推力曲线来自 AFSIM 默认配置文件（Mil 推力表 Imperial→SI 转换）；m_dot_const 和 Max_Fuel_Capacity 通过 `para.getPara()` 获取 |

**综合等级**：低  
**剥离策略**：1D 推力曲线 T(altitude) 实现。推力随高度连续变化，比最简常量模式更精细。
...
- ~~需人工补充：`T_max(h)` 曲线数据（建议使用 AFSIM 默认 Mil 推力表经 Imperial→SI 转换）；`ṁ_const` 和 `Max_Fuel_Capacity` 参数值~~ **✅ 已确认**：所有数据库来源参数通过 `para.getPara()` 读取（如 `para.getPara("m_dot_const", 0.0)`、`para.getPara("Max_Fuel_Capacity", 0.0)`），参照 target-interfaces.md
- **优化方向**：
...
- **✅ 已确认——单位策略**：若 SI 单位转换后需要重调，本 FU（FU-006）内部保留 Imperial 单位，仅在对外接口 FU-009 处进行单位转换。调参沿用 AFSIM 默认增益表无需修改
- **技术风险（高）**：增益调度表以动压 q̄ 为主控变量，在低速段（动压低→增益高）可能产生控制过冲
- **✅ 已确认——低速段增益过冲解决方案**：
  1. **增益限幅**：在增益调度查表后增加 `K_min` 下限保护——当 $\bar{q} < \bar{q}_{min\_threshold}$ 时，$K = \max(K_{scheduled}, K_{min\_safe})$，其中 $K_{min\_safe}$ 为静态安全增益集（每个 PID 独立配置），防止动压过低时增益放大到不稳定区域
  2. **动压下界 clamp**：增益调度控制变量 $\bar{q}_{clamped} = \max(\bar{q}, \bar{q}_{min})$，其中 $\bar{q}_{min}$ 为最小有效动压（如 50 Pa，对应约 10 m/s 海平面），低于此值时使用固定增益不随速度继续放大
  3. **控制面速率限制**：在 PID 输出端增加速率限制器（rate limiter），$\Delta \delta_{max} = \dot{\delta}_{max} \cdot \Delta t$，防止低速段单帧指令跳变过大
...
flowchart TD
    ...
    B --> C["① 加载/更新配置参数<br/>lonwind, latwind, altwind, V_max, m_const, ..."]
    C --> D["② PATH-01管线"]
...
| 2 | `lonwind` | `double` | `params` 参数集 | 任意实数 m/s | 经向风（南北风向，北为正），通过 `para.getPara("V_wind_lon", 0.0)` 读取 |
| 3 | `latwind` | `double` | `params` 参数集 | 任意实数 m/s | 纬向风（东西方向，东为正），通过 `para.getPara("V_wind_lat", 0.0)` 读取 |
| 4 | `altwind` | `double` | `params` 参数集 | 任意实数 m/s | 垂直风（沿重力方向，向上为正），通过 `para.getPara("V_wind_alt", 0.0)` 读取 |
| 5 | `V_max` | `double` | `params` 参数集 | (0, 500] m/s | 最大速度 |
| 6 | `m_const` | `double` | `params` 参数集 | (0, ∞) kg | 飞行器质量常量 |
| 7 | `Max_Fuel_Capacity` | `double` | `params` 参数集 | (0, 50000] kg | 油箱最大容量 |
| 8 | `S_ref` | `double` | `params` 参数集 | (0, 1000] m² | 参考面积 |
| 9 | `l_ref` | `double` | `params` 参数集 | (0, 100] m | 参考长度 |
...
| 评估维度 | 说明 |
|----------|------|
| 框架耦合 | 低——遵循 target-interfaces.md 定义的 C++ 类接口（CMRBasicBAC 组件指针 + std::any 参数），但不依赖 AFSIM 核心引擎 |
| 数据耦合 | 高——依赖全部 8 个子 FU 的输入/输出数据流（每个 FU 的输入来源和输出去向均在本集成层定义） |
| 控制耦合 | 高——严格顺序调度 PATH-01 → KINEMATICS-02 两级管线，各子 FU 之间有严格的前置/后置依赖 |
| 外部依赖 | 中——依赖配置参数集（`std::unordered_map<std::string, std::any>`）和仿真时间 |
| **综合等级** | **中** |
...
| `params_` | `std::unordered_map<std::string, std::any>*` | nullptr | 对象级 | 否 | 参数集容器指针 |
